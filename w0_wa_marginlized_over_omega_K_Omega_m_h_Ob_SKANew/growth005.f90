  ...
  SUBROUTINE Setup_Growth005
    ...
    do i=1,nw
       ...
       if(i.eq.1)then
          ...
          call dverk(2,fderiv005,xini,y,x,tol,ind,c,2,work)
          xini = x
       endif
       ga(i) = y(1)
       dga(i)= y(2)
    ...
    return
  END SUBROUTINE Setup_Growth005
END MODULE Growth005
!-----------------------------------------------------------------------------
SUBROUTINE fderiv005(n,x,y,yprime)
  USE cosmo
! x = ln(a) = -ln(1+z)
! y(1) = g
! y(2) = g'
! See Eq.(76) of Komatsu et al. (2008) [WMAP 5-year]
  ...
  z=dexp(-x)-1d0
  a=dexp(x)
  ok0=1d0-om0-ode0-ob005
  h2=(om0+ob005)/a**3d0+or0/a**4d0+ok0/a**2d0+ode0/a**(3d0*(1.+w))
  ...
  return
END SUBROUTINE fderiv005
!-----------------------------------------------------------------------------
DOUBLE PRECISION FUNCTION g005(z)
  USE growth005
  ...
  a=(xa(jlo+1)-x)/hh
  b=(x-xa(jlo))/hh
  g005=a*ga(jlo)+b*ga(jlo+1)+((a**3-a)*g2a(jlo)+(b**3-b)*g2a(jlo+1))*(hh**2)/6.
  return
END FUNCTION g005
!-----------------------------------------------------------------------------
DOUBLE PRECISION FUNCTION dgdlna005(z)
  USE growth005
  ...
  a=(xa(jlo+1)-x)/hh
  b=(x-xa(jlo))/hh
  dgdlna005=a*dga(jlo)+b*dga(jlo+1)+((a**3-a)*dg2a(jlo)+(b**3-b)*dg2a(jlo+1))*(hh**2)/6.
  return
END FUNCTION dgdlna005
