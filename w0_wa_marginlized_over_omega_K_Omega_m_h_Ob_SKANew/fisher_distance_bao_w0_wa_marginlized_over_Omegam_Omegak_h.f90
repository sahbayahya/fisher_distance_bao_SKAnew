     ...
     do while (k_ov_h<=kmax_ov_h)
       ...
       ! print*, factor
        ...
        do while (mu<=1d0)
           ...
           pk=linear_pk(k_ov_h)*(g(z)/g(zin)*(1d0+zin)/(1d0+z))**2d0 &
            ...
            !P01  P(k) at k=0.1 h Mpc^-1  in units of h^-3 Mpc^3  
              p01=linear_pk(k_ov_h)*(g(z)/g(zin)*(1d0+zin)/(1d0+z))**2d0&
                *bias**2d0!*(1d0+beta*mu2)**2d0
           !==========================================     
           wkmu=0.5*Vsurvey*(ngal/(1d0+ngal*pk))**2d0
           ...
           *  wdamp_para *  wdamp_silk * exp(-(k_ov_h*sigma_z)**2d0*mu2)
        ...
        !   print*, der(1), der(2)
           do i=1,npara
                ...
                *(8d0*3.1415926535d0**2d0 * BAO_AMP**2d0 * p02**2d0)*wdamp ! factor*wkmu!
           enddo
           mu=mu+dmu
        enddo
       !================= save files ====================================== 
     !  open(5,file='Pk_Euclid_testPlank_2.txt' ,status='unknown')
